...
def get_info_nb_values(name_var: int):
    if name_var in {0, 1, 2, 3, 4, 5, 6, 7}:
        return 100
    else:
        return 1
...
def run_simulator(variable_interest: str, variable_value_begin: int, variable_value_end: int,
    ...
    while variable_value_begin <= var_value <= variable_value_end:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        for argument in sys.argv:
            ...
            elif option[0] == "-number_of_simulation":
                nb_simulation = int(option[1])
    ...
    if name_variable in {0, 1, 2, 3, 4, 5, 6, 7}:
        run_simulator(variable_interest=var_interest, variable_value_begin=var_value_begin,
                      ...
                      number_of_simulation=nb_simulation, list_other_interest_var=list_other_parameter_simulator,
